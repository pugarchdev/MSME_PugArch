...
export default function Sidebar({ isOpen, onClose, isCollapsed, onToggleCollapse, onHoverChange }: SidebarProps) {
  ...
  const isShgAccount = user?.role === 'seller' && String(user?.registrationDetails?.shgType || user?.registrationDetails?.businessType || '').toLowerCase() === 'hershg';
  ...
  const navItems: SidebarItem[] = useMemo(() => [
    ...
    { label: isShgAccount ? 'SHG Hub' : 'Seller Hub', path: '/seller/onboarding', icon: isShgAccount ? UsersRound : Store, roles: ['seller'] },
...
export function Header({ onMenuClick, onSidebarToggle, isSidebarCollapsed }: HeaderProps) {
  ...
  const isShgAccount = user?.role === 'seller' && String((user as any)?.registrationDetails?.shgType || (user as any)?.registrationDetails?.businessType || '').toLowerCase() === 'hershg';
