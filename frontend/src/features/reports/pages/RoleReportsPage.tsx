...
export default function RoleReportsPage() {
    ...
    const handleExport = (type: 'csv' | 'json' | 'print') => {
        ...
        if (type === 'json') {
            downloadFile(`${filename}.json`, JSON.stringify({ summary: analytics.kpis, orders: exportRows }, null, 2), 'application/json');
            return;
        }
        const headers = Object.keys(exportRows[0] || { poNumber: '', title: '', buyer: '', seller: '', amount: '', status: '', delivery: '', grn: '', invoice: '', payment: '', createdAt: '' });
        const csv = [
            headers.join(','),
            ...exportRows.map((row) => headers.map((key) => csvCell((row as any)[key])).join(',')),
        ].join('\n');
